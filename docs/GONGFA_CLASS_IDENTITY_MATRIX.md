...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Fire | Blazing Feather Art | Youxia | Marksman | An automatic feather fan uses its narrow outer edge as the precision zone | Perfect-range edge hits load Molten Pinions; misses break the cadence | Phoenix Horizon: one automatically selected execution corridor crosses every marked enemy in line | Yujian homing, Vermilion companion, Scarlet waves |
...
| Water | Drifting Frost Needle | Youxia | Marksman | One automatic needle ricochets only from a struck exposed weak point | Consecutive hits on different weak points build Focus; repeating one target resets it | Winter Thread: the current needle immediately retraces its weak-point chain in reverse | Yujian sword ammo, Blazing fan, Ice Mirror reflection |
...
| Water | Frozen River Formation | Youxuan | Warlock | Curse selected routes; the river awakens only when a cursed enemy crosses another seal | Each crossing transfers Cold Debt between enemies; damage alone produces nothing | Frozen River Prison: debtors are linked into one shared frozen fate | Myriad-Root infection, Sword Burial corpses, ordinary pulsing traps |
...
| Metal | Yujian Jue | Youxia | Marksman / weapon master | Command a limited rack of flying swords; launched swords must return before reuse | Sword-Rack Rotation comes from assigning the right ready sword, not from hit stacks | Myriad Swords Return: all airborne swords reverse through their exact paths | Frost ricochet, Blazing fan, generic homing volleys |
...
| Metal | Gengjin Huti | Hudao | Forge Guard | Deliberately absorb close hits behind a forged guard and answer at contact range | Guard stores exact prevented damage, with a hard capacity and visible fracture states | Blade-Shell Rebound: conserve the stored total, then divide it, focus it once, or convert it into a shield | Ice Mirror interception, Ironwood stance, Flame Demon missing health |
...
| Wood | Myriad-Root Lifebinding Canon | Youxuan | Warlock | Implant parasitic root curses inside living targets rather than placing floor traps | Infected enemies grow roots over time and spread one seed when they die | Myriad Roots Share One Ancestor: all infections erupt simultaneously and become one organism | Sword Burial corpses, Frozen River crossings, Green Vine tethers |
